...
$header_qry = $db->prepare('SELECT DISTINCT groups.id, groups.label FROM packages JOIN groups ON packages.group_id = groups.id WHERE distro_id = ?');
...
$i = 0;
$group_qry = $db->prepare('SELECT DISTINCT groups.id, groups.label FROM packages JOIN groups ON packages.group_id = groups.id WHERE distro_id = ?');
...
<div class="list-group">
  <div class="list-group-item col-sm-4">
    ...
    <div class="row-content">
      <h4 class="list-group-item-heading">$pkg[label]</h4>
      <p class="list-group-item-text">$pkg[desc]</p>
    </div>
  </div>
</div>
<div class="list-group-separator"></div>
