...
<html>
  ...
  <body>
    ...
    <div id="btn-drawer" class="pull-right">
      <a id="new-btn"  class="btn btn-primary btn-fab btn-fab-float mdi-content-add"    tooltip-title="New Package" data-toggle="modal" data-target="#nc-modal"></a>
          ...
          <div class="modal-body">
            ...
            <form id="new-form">
              <fieldset>
                  ...
                  <div class="form-group">
                    <input class="form-control floating-label" id="focusedInput" type="text"     required placeholder="Package name"  name="pkg_name"    data-hint="Please enter the package name">
                  </div>
                  <div class="form-group">
                    <input class="form-control floating-label" id="focusedInput" type="text"     required placeholder="Friendly name" name="label"  data-hint="Please enter a display name">
                  </div>
                  <div class="form-group">
                    <input class="form-control floating-label" id="focusedInput" type="textarea" required placeholder="Description"   name="desc" data-hint="Please enter a description for the package">
                  </div>
...
<?php
$sql = "select packages.id, distro_id, name, groups.label as group_label, group_id, pkg_name, packages.label as pkg_label, packages.desc from packages join distros on packages.distro_id = distros.id join groups on packages.group_id = groups.id";
$results = $db->query($sql);
$i=0;
foreach ($results as $row) {
    ...
    echo sprintf('<td><span data-name="group_id"  data-pk="%s" data-type="select"   class="xedit" data-value="%s" data-source="groups.php">%s</span></td>',  $row["id"], $row["group_id"],  $row["group_label"]);
    echo sprintf('<td><span data-name="pkg_name"  data-pk="%s" data-type="text"     class="xedit">%s</span></td>', $row["id"], $row["pkg_name"]);
    echo sprintf('<td><span data-name="pkg_label" data-pk="%s" data-type="text"     class="xedit">%s</span></td>', $row["id"], $row["pkg_label"]);
    echo sprintf('<td><span data-name="desc"      data-pk="%s" data-type="textarea" class="xedit">%s</span></td>', $row["id"], $row["desc"]);
      ...
      function createRow(data){
        if (data && data.id) { //record created, response like {"id": 2}
          var new_row = $('<tr rowid="'+data.id+'">' +
            ...
            '<td><span data-name="group"    data-pk="'+data.id+'" data-type="select"   class="xedit" data-value="'+data.distro_id+'" data-source="groups.php">'+data.group+'</span></td>' +
            '<td><span data-name="pkg_name" data-pk="'+data.id+'" data-type="text"     class="xedit">'+data.pkg_name   +'</span></td>' +
            '<td><span data-name="label"    data-pk="'+data.id+'" data-type="text"     class="xedit">'+data.label +'</span></td>' +
            '<td><span data-name="desc"     data-pk="'+data.id+'" data-type="textarea" class="xedit">'+data.desc+'</span></td>' +
